...
export default {
  ...
  methods: {
    ...
    loadDataList: function () {
      ...
      this.$http('approval/searchTaskByPage', 'POST', data, true, res => {
        let page = res.page;
    ...
    //获取审批任务详细信息
    expand: function (row, expandRow) {
      if (expandRow.length > 0) {
        this.expands = []; //关闭所有折叠面板
        this.archiveList = [];
        if(row){
          this.expands.push(row.taskId);
          //获取BPMN图
          this.bpmnUrl = this.$baseUrl + 'approval/searchApprovalBpmn?instanceId=' + row.processId + '&time=' + new Date().getTime();
          this.bpmnList = [this.bpmnUrl];
          //获取审批任务详细信息
          let data = {
          ...
          this.$http('approval/searchApprovalContent', 'POST', data, false, res => {
            this.content = res.content;
            if(res.content.hasOwnProperty('files')){
                for(let item of res.content.files){
                    this.archiveList.push(item.url)
                }
            }
          })
        }
