  ...
  methods: {
    ...
    dataFormSubmit: function () {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          //用户选择权限排序
          this.dataForm.permissions.sort((a, b) => {
            return a - b;
          });
          //判断新老权限是否一致
          if (this.dataForm.permissions.join() != this.oldPermissions.join()) {
            this.dataForm.changed = true
          } else {
            this.dataForm.changed = false
          }
          this.$http(`role/${!this.dataForm.id ? 'insertRole' : 'updateRole'}`, 'POST', this.dataForm, true, res => {
            if (res.rows == 1) {
              this.$message({
                message: '操作成功',
                type: 'success',
