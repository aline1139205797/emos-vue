...
export default {
  ...
  methods: {
    ...
    loadDataList: function () {
      ...
      this.$http("meeting/searchOfflineMeetingByPage", "POST", data, true, res => {
        let page = res.page;
        let temp = [];
        for (let room of page.list) {
          ...
          if (room.hasOwnProperty("meeting")) {
            for (let meetingObj of room.meeting) {
              let color;
              switch (meetingObj.status) {
                case 1:
                  color = "yellow";
                  break;
                case 3:
              ...
              json.meeting[meetingObj.start] = meetingObj.time + '#' + color;
            }
          }
          temp.push(json);
        }
      ...
      if (this.dataForm.name == null || this.dataForm.name == "") {
        ...
        this.$refs['dataForm'].validate(valid => {
          if (valid) {
            ...
            this.$http("meeting/searchOfflineMeetingInWeek", "POST", data, true, res => {
              let map = {};
              for (let item of res.list) {
                map[`${item.date}#${item.start}`] = item;
              }
              this.calendar.map = map;
              this.calendar.days = res.days;
              this.mode = 'calendar';
            })
          } else {
            return false;
